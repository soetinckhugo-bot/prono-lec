    ...
    <div className="mx-auto max-w-5xl">
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-white/10 bg-surface/60 p-5 backdrop-blur-xl">
          <p className="text-sm text-white">Total pronos</p>
          <p className="mt-1 text-3xl font-black text-white">{data.splitStats.total}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-surface/60 p-5 backdrop-blur-xl">
          <p className="text-sm text-white flex items-center gap-1"><CheckCircle2 className="h-4 w-4 text-primary" /> Bons</p>
          <p className="mt-1 text-3xl font-black text-primary">{data.splitStats.correctPct}%</p>
          <p className="text-xs text-white">{data.splitStats.correct} / {data.splitStats.total}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-surface/60 p-5 backdrop-blur-xl">
          <p className="text-sm text-white flex items-center gap-1"><Target className="h-4 w-4 text-warning" /> Exact</p>
          <p className="mt-1 text-3xl font-black text-warning">{data.splitStats.exactPct}%</p>
          <p className="text-xs text-white">{data.splitStats.exact} / {data.splitStats.total}</p>
        </div>
        <div className="rounded-2xl border border-white/10 bg-surface/60 p-5 backdrop-blur-xl">
          <p className="text-sm text-white flex items-center gap-1"><XCircle className="h-4 w-4 text-danger" /> Faux</p>
          <p className="mt-1 text-3xl font-black text-danger">{data.splitStats.wrongPct}%</p>
          <p className="text-xs text-white">{data.splitStats.wrong} / {data.splitStats.total}</p>
        </div>
      </div>
      ...
      <section>
        ...
        {data.history.length === 0 ? (
          <p className="text-white">Aucun prono enregistré.</p>
        ) : (
                  ...
                  <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="text-right">
                      {isFinished ? (
                        ...
                        <span className="text-sm text-white">En attente</span>
                      )}
                    </div>
                  </div>
                </div>
